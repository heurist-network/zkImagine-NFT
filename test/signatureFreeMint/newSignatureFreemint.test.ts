...
describe("ZkImagine Contract", function () {
  ...
  beforeEach(async function () {
    ...
    zkImagine = await hre.zkUpgrades.deployProxy(
      owner,
      artifact,
      [name, symbol, baseTokenURI, mintFee.toString(), referralDiscount.toString(), COOLDOWN_WINDOW.toString(), timestamp.toString()],
  ...
  it("Should allow minting with a valid signature", async function () {
    ...
    const tx = await zkImagine.connect(user1).signatureFreeMint(hash, signature, "model-id-1", "image-id-1");
    await tx.wait();
  ...
  it("Should not allow minting with an invalid hash", async function () {
    ...
    const signature = await owner.signMessage(ethers.getBytes(invalidHash));

    await expect(zkImagine.connect(user1).signatureFreeMint(invalidHash, signature, "model-id-2", "image-id-2"))
  ...
  it("Should not allow minting twice within cooldown window using the same signature", async function () {
    ...
    // First mint
    await zkImagine.connect(user2).signatureFreeMint(hash, signature, "model-id-4", "image-id-4");

    // Attempt second mint
    await expect(zkImagine.connect(user2).signatureFreeMint(hash, signature, "model-id-5", "image-id-5"))
...
//   });
  ...
  it("Should not allow non-owner to sign valid minting messages", async function () {
    ...
    await expect(zkImagine.connect(user1).signatureFreeMint(hash, signature, "model-id-10", "image-id-10"))
